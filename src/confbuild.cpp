...
std::string base_vless = R"({"inbounds":[{"port":?localport?,"listen":"127.0.0.1","protocol":"socks","sniffing":{"enabled":true,"destOverride":["http","tls"]},"settings":{"auth":"noauth","udp":true,"allowTransparent":false}}],"outbounds":[{"tag":"proxy","protocol":"vless","settings":{"vnext":[{"address":"?add?","port":?port?,"users":[{"id":"?id?","alterId":?aid?,"email":"t@t.tt","security":"?cipher?","encryption": "none","flow": "?flow?"}]}]},"streamSettings":{"network":"?net?","security":"?tls?","?tlsSettings?":?tlsset?,"tcpSettings":?tcpset?,"wsSettings":?wsset?,"kcpSettings":?kcpset?,"httpSettings":?h2set?,"quicSettings":?quicset?,"grpcSettings":?grpcset?},"mux":{"enabled":false}}],"routing":{"domainStrategy":"IPIfNonMatch","domainMatcher": "linear","rules": [{"type": "field", "port": "0-65535","outboundTag": "proxy","enabled": true}]}})";
...
std::string vlessConstruct(const std::string &group, const std::string &remarks, const std::string &add, const std::string &port, const std::string &type, const std::string &id, const std::string &aid, const std::string &net, const std::string &cipher, const std::string &flow, const std::string &sni, const std::string &mode, const std::string &path, const std::string &host, const std::string &edge, const std::string &tls, tribool udp, tribool tfo, tribool scv, tribool tls13)
{
    ...
    base = replace_first(base, "?tls?", tls);
    base = replace_first(base, "?tlsSettings?", tls == "xtls" ? "xtlsSettings" : "tlsSettings");
    base = replace_first(base, "?tcpset?", "null");
    base = replace_first(base, "?wsset?", "null");
    base = replace_first(base, "?tlsset?", "null");
    base = replace_first(base, "?kcpset?", "null");
