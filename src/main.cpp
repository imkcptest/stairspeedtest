...
void clientCheck()
{
    ...
    std::string xray_path = "tools/clients/xray";
    ...
    if(fileExist(xray_path))
    {
        avail_status[SPEEDTEST_MESSAGE_FOUNDVLESS] = 1;
        writeLog(LOG_TYPE_INFO, "Found xray core at path " + xray_path);
    }
    else
    {
        avail_status[SPEEDTEST_MESSAGE_FOUNDVLESS] = 0;
        writeLog(LOG_TYPE_WARN, "xray core not found at path " + xray_path);
    }
